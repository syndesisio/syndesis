...
In order to have this script handy all the time it is recommended to either but this `bin/` directory into the path or add symlink from `syndesis` into a directory which is already on your execution path.

The script can be used for various task which are selected by a so called _subcommand_ which is the first argument to the script.
...
| `--rebase`
| `-r`
| Rebase you project directory before building to integrate upstream changes to your local repository. See <<syndesis-rebase,below>> for details how this works.
...
[[syndesis-rebase]]
.Rebase to upstream
In order to easily rebase on changes which has been merged upstream to master, you can use the option `--rebase` (short: `-r`).
...
* If this fails because you have uncommitted work:
  ...
  - `git stash pop` brings back your changes. This can fail with conflicts which you would have to resolve on your own.
...
Actually, you can choose from two different modes how Docker images are installed and updated:

.Docker mode
The Docker mode avoid OpenShift specific image stream and used plain Docker image references in the deployment descriptors.
Without an extra Docker registry push, this only works smoothely on single node OpenShift installation like with Minishift or `oc cluster up`.
For Minishift, the Docker daemon exposed with `minishift docker-env` is used.
The advantage of this mode is, that building is a bit faster and you avoid the overhead of image streams.
On the other side you have to kill pods when images are upated so that the new images are picked up.

.OpenShift mode
The OpenShift mode uses OpenShift S2I builds and imagestreams.
This works also when running with a real OpenShift cluster and is not restricted on Minishift usage.
...
On the con side is a slightly slower build.
...
| <<syndesis-deploy,**deploy**>>
| Deploy Syndesis to a running cluster (other than Minishift) _(not implemented yet)_
...
* You can use the form `--command` anywhere as option, too. E.g. using  `--minishift` is the same as specifyin "minishift" as first argument.
* A `--mode command` can be used, too (e.g. `--mode sytem-test`)

The next sections describe now the commands in detail.
In order to add a new command, just drop a script file into
