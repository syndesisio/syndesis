...
# Push to origin after a successful release (or add --git-push)
git push
git push -f --tags
...
Finally, if the option `--git-push` is given the local Git repo is pushed to the remote repository.
This last step can also be performed manually afterwards with:
...
git push -f --tags <1>
----
<1> Using `-f` as the minor tag needs to be moved. Alternatively first remove the minor tag and the use push without `--tags`
