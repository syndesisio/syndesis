...
Options for build:
    -b  --backend                  Build only backend modules (core, extension, integration, connectors, server, meta, dv)
        --all-images               Build all modules with images: ui, server, meta, s2i, operator, upgrade, camel-k, dv
        --app-images               Build only application modules with Docker images (ui, server, meta, s2i, dv)
                                   and create images
    ...
    -m  --module <m1>,<m2>, ..     Build modules
                                   Top level modules: ui, server, connector, s2i, meta, integration, extension, common, operator, camel-k, upgrade, dv
...
| **dv**
| DV image provides the backend services to Syndesis UI to support the Data Virtualization feature
| ✔︎
| ✔︎
| ✔︎
|
