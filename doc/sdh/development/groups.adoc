...
E.g. the directory `app/integration/api` holds a Maven module for the the Maven group `io.syndesis.integration`, and the module is called `integration-api`.
* A module's directory name is directly reflected as the last part of the Maven module name.
If the Maven module name consists of multiple parts (e.g. like for `project-generator` in directory `app/integration`), then the corresponding directory is also a concatenated word `integration/project-generator`.
* There should be only one level deep modules, so each Maven group directory holds all Maven module directory flat.
