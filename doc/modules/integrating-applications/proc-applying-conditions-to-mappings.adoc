...
. Optionally, apply transformations to one or more source or target fields in the selected mapping by clicking image:images/integrating-applications/lightning-bolt-icon.png[Lightning bolt icon] next to each field to which you want to apply a transformation and then selecting the desired transformation from the pull-down menu.
+
