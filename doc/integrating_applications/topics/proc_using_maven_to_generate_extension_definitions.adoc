...
= Description of Maven plugin for generating an extension definition

The `extension-maven-plugin` provides extension support by
generating the JSON extension definition and packaging the
extension as a valid Spring Boot module.

In a Maven `pom.xml` file, the entry for the extension support plugin 
looks like this:
...
* *generate-metadata* generates the JSON extension definition 
according to the following rules:
+
** Includes specifications from 
`META-INF/syndesis/syndesis-extension-definition.json` to generate the 
final descriptor.
  - information such as _id_, _name_, _description_ and _version_ are computed from the project if not explicit 
  configured through plugin configuration or using `META-INF/syndesis/syndesis-extension-definition.json`
