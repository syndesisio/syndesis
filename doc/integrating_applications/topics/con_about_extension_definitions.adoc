...
= Requirements in an extension definition

A `.json` file defines an extension by specifying values for
data structures such as name, description, supported actions, and
dependencies. Typically, an extension definition file has the following
layout: 
...
at least across a Syndesis instance. 
...
----
** *id* is a unique ID for the action.
** *name* is the action name that appears in {prodname}. An integrator
sees this name as a possible selection. 
...
----
** *type* indicates the type of the dependency. Specify *MAVEN*,
*EXTENSION*, or *EXTENSION_TAG*.
** *id* is the ID of the dependency. For Maven dependencies, it is a Maven GAV. 
