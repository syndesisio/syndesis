...
For each parameter, specify `feet`, `meters`, or `miles`.
...
`fromUnit` and `toUnit` parameters, specify `feet`, `inches`, `meters`, `miles`, 
or `yards`.
...
`fromUnit` and `toUnit` parameters, specify `kilograms` or `pounds`. 
...
gallons, or liters. For each parameter, specify 
