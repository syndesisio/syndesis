...
| string
| `template` *
| Return a string that is the result of substituting a field's value within
a template that contains placeholders such as `%s`, `%d`, and so on. This
...
| number
| None
| For each entry in the set, subtract it from the next entry, if there is one. 
A normal subtraction operates on a set that has two values. 
