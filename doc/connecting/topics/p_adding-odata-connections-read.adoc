...
An OData connection can obtain data from only one resource. As described in 
the OData 4 specification, an OData resource is identified by a URL and it 
defines a data model. For an example, see the resource at the following URL:

link:https://services.odata.org/TripPinRESTierService/(S(yvqxbruqidpd1qecd35rwl5o))/$metadata[]
