...
a Webhook connection to an integration. Each integration that
starts with a Webhook connection has its own token. 
...
* The `body` section must specify the body content. 
...
`curl -H 'Content-Type: application/json' -d '{"company":"Gadgets","email":"email","phone":"phone"}'https://i-webhook-params-to-db-myproject.192.168.42.235.nip.io/webhook/ZYWrhaW7dVk097vNsLX3YJ1GyxUFMFRteLpw0z4O69MW7d2Kjg?source=web&status=new`
