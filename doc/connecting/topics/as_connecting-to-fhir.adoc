...
* Can operate on all FHIR resources that comply with FHIR DSTU3, with these limitations:
** Connections to FHIR cannot operate on extensions, which allow for storing custom data in a resource. 
If an extension is in data that is received from a FHIR server, the integration
silently ignores it. 
** In a data mapper step in a {prodname} integration, you can map only 
one level of depth for a list field. In other words, if a list contains 
a list then you cannot map the fields in the nested list. To mitigate 
this limitation, {prodname} explicitly converts some FHIR resource 
