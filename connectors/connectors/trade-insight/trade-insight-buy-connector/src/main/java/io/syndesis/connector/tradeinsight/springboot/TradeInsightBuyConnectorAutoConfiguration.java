...
import io.syndesis.connector.tradeinsight.TradeInsightBuyComponent;
...
public class TradeInsightBuyConnectorAutoConfiguration {
    ...
    private static final Logger LOGGER = LoggerFactory
            .getLogger(TradeInsightBuyConnectorAutoConfiguration.class);
    @Autowired
    private ApplicationContext applicationContext;
    @Autowired
    private CamelContext camelContext;
    @Autowired
    private TradeInsightBuyConnectorConfiguration configuration;
    @Autowired(required = false)
    private List<ConnectorCustomizer<TradeInsightBuyComponent>> customizers;
    ...
    @PostConstruct
    public void postConstructTradeInsightBuyComponent() {
        Map<String, Object> parameters = new HashMap<>();
                    ...
                    for (ConnectorCustomizer<TradeInsightBuyComponent> customizer : customizers) {
                        boolean useCustomizer = (customizer instanceof HasId)
                                ? HierarchicalPropertiesEvaluator.evaluate(
                                        applicationContext.getEnvironment(),
                                        "camel.connector.customizer",
                                        "camel.connector.trade-insight-buy."
                                                + entry.getKey()
                                                + ".customizer",
                                        ((HasId) customizer).getId())
                                : HierarchicalPropertiesEvaluator.evaluate(
