...
public class HttpGetConnectorAutoConfiguration {
    ...
    private static final Logger LOGGER = LoggerFactory
            .getLogger(HttpGetConnectorAutoConfiguration.class);
    @Autowired
    private ApplicationContext applicationContext;
    @Autowired
    private CamelContext camelContext;
    @Autowired
    private HttpGetConnectorConfiguration configuration;
    @Autowired(required = false)
    private List<ConnectorCustomizer<HttpGetComponent>> customizers;
    ...
    @PostConstruct
    public void postConstructHttpGetComponent() {
        Map<String, Object> parameters = new HashMap<>();
                    ...
                    for (ConnectorCustomizer<HttpGetComponent> customizer : customizers) {
                        boolean useCustomizer = (customizer instanceof HasId)
                                ? HierarchicalPropertiesEvaluator.evaluate(
                                        applicationContext.getEnvironment(),
                                        "camel.connector.customizer",
                                        "camel.connector.http-get-connector."
                                                + entry.getKey()
                                                + ".customizer",
                                        ((HasId) customizer).getId())
                                : HierarchicalPropertiesEvaluator.evaluate(
