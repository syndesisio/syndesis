...
import io.syndesis.connector.salesforce.SalesforceOnCreateComponent;
...
@EnableConfigurationProperties(SalesforceOnCreateConnectorConfiguration.class)
public class SalesforceOnCreateConnectorAutoConfiguration {

    private static final Logger LOGGER = LoggerFactory
            .getLogger(SalesforceOnCreateConnectorAutoConfiguration.class);
    @Autowired
    private ApplicationContext applicationContext;
    @Autowired
    private CamelContext camelContext;
    @Autowired
    private SalesforceOnCreateConnectorConfiguration configuration;
    @Autowired(required = false)
    private List<ConnectorCustomizer<SalesforceOnCreateComponent>> customizers;
    ...
    @PostConstruct
    public void postConstructSalesforceOnCreateComponent() {
        Map<String, Object> parameters = new HashMap<>();
                    ...
                    for (ConnectorCustomizer<SalesforceOnCreateComponent> customizer : customizers) {
                        boolean useCustomizer = (customizer instanceof HasId)
                                ? HierarchicalPropertiesEvaluator.evaluate(
                                        applicationContext.getEnvironment(),
                                        "camel.connector.customizer",
                                        "camel.connector.salesforce-on-create."
                                                + entry.getKey()
                                                + ".customizer",
                                        ((HasId) customizer).getId())
                                : HierarchicalPropertiesEvaluator.evaluate(
