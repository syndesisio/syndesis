...
import io.syndesis.connector.salesforce.SalesforceGetSObjectComponent;
...
@EnableConfigurationProperties(SalesforceGetSObjectConnectorConfiguration.class)
public class SalesforceGetSObjectConnectorAutoConfiguration {

    private static final Logger LOGGER = LoggerFactory
            .getLogger(SalesforceGetSObjectConnectorAutoConfiguration.class);
    @Autowired
    private ApplicationContext applicationContext;
    @Autowired
    private CamelContext camelContext;
    @Autowired
    private SalesforceGetSObjectConnectorConfiguration configuration;
    @Autowired(required = false)
    private List<ConnectorCustomizer<SalesforceGetSObjectComponent>> customizers;
    ...
    @PostConstruct
    public void postConstructSalesforceGetSObjectComponent() {
        Map<String, Object> parameters = new HashMap<>();
        for (Map.Entry<String, SalesforceGetSObjectConnectorConfigurationCommon> entry : configuration
                    ...
                    for (ConnectorCustomizer<SalesforceGetSObjectComponent> customizer : customizers) {
                        boolean useCustomizer = (customizer instanceof HasId)
                                ? HierarchicalPropertiesEvaluator.evaluate(
                                        applicationContext.getEnvironment(),
                                        "camel.connector.customizer",
                                        "camel.connector.salesforce-get-sobject."
                                                + entry.getKey()
                                                + ".customizer",
                                        ((HasId) customizer).getId())
                                : HierarchicalPropertiesEvaluator.evaluate(
