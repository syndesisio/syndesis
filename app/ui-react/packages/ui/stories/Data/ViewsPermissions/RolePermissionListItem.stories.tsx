...
const stories = storiesOf('Data/ViewsPermissions/RolePermissionListItem', module);

const addRoleActionText = 'Add role action';
const updateRolePermissionModelActionText = 'Update role permission model action';
const deleteRoleFromPermissionModelActionText = 'Delete role from permission action';
...
const sampleRolePermissionItemNotes =
  ...
  '- Verify typing a new role name in the editable dropdown will show a "' + 'create role" action in the dropdown\n' +
  '- Verify clicking the create role action prints "' + addRoleActionText + '" in the ACTION LOGGER\n' +
  '- Verify clicking any of the five checkboxes prints "' + updateRolePermissionModelActionText + '" in the ACTION LOGGER\n' +
  '-   the update should include the roleName with an array of the checked boxes (SELECT, INSERT, UPDATE, DELETE)\n' +
  '- Verify clicking on the remove row icon prints "' + removeRolePermissionActionText + '" in the ACTION LOGGER'; 
    ...
    <Grid>
      <GridItem span={10}>
        <RolePermissionListItem
          ...
          addRole={action(addRoleActionText)}
