...
import kudu.component.api.KuduTablesManager;
import kudu.component.internal.*;
 ...
 */
@UriEndpoint(firstVersion = "1.0-SNAPSHOT", scheme = "kudu", title = "Kudu", syntax="kudu:apiName/methodName",
        consumerClass = KuduConsumer.class, label = "database")
public class KuduEndpoint extends AbstractApiEndpoint<KuduApiName, KuduConfiguration> {

    @UriParam
    private KuduConfiguration configuration;
    ...
    private void createKuduConnection() {
        ...
        this.clientShared = configuration.equals(getComponent().getConfiguration());
        if (clientShared) {
            // get shared singleton connection from Component
            this.client = component.getClient();
        } else {
            this.client = KuduConnectionHelper.createConnection(configuration);
        }
    }
    ...
    private void createApiProxy(Map<String, Object> args) {
        switch (apiName) {
            case TABLES:
                apiProxy = new KuduTablesManager(getClient());
                break;
            default:
