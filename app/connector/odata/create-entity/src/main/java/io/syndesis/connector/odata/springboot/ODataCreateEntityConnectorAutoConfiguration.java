...
import io.syndesis.connector.odata.ODataCreateEntityComponent;
...
@EnableConfigurationProperties(ODataCreateEntityConnectorConfiguration.class)
public class ODataCreateEntityConnectorAutoConfiguration {

    private static final Logger LOGGER = LoggerFactory
            .getLogger(ODataCreateEntityConnectorAutoConfiguration.class);
    @Autowired
    private ApplicationContext applicationContext;
    @Autowired
    private CamelContext camelContext;
    @Autowired
    private ODataCreateEntityConnectorConfiguration configuration;
    @Autowired(required = false)
    private List<ConnectorCustomizer<ODataCreateEntityComponent>> customizers;
    ...
    public void postConstructODataCreateEntityComponent() {
        Map<String, Object> parameters = new HashMap<>();
        for (Map.Entry<String, ODataCreateEntityConnectorConfigurationCommon> entry : configuration
                    ...
                    for (ConnectorCustomizer<ODataCreateEntityComponent> customizer : customizers) {
                        boolean useCustomizer = (customizer instanceof HasId)
                                ? HierarchicalPropertiesEvaluator.evaluate(
                                        applicationContext.getEnvironment(),
                                        "camel.connector.customizer",
                                        "camel.connector.odata-create-entity."
                                                + entry.getKey()
                                                + ".customizer",
                                        ((HasId) customizer).getId())
                                : HierarchicalPropertiesEvaluator.evaluate(
