...
import io.syndesis.connector.activemq.ActiveMQPublishComponent;
...
@EnableConfigurationProperties(ActiveMQPublishConnectorConfiguration.class)
public class ActiveMQPublishConnectorAutoConfiguration {

    private static final Logger LOGGER = LoggerFactory
            .getLogger(ActiveMQPublishConnectorAutoConfiguration.class);
    @Autowired
    private ApplicationContext applicationContext;
    @Autowired
    private CamelContext camelContext;
    @Autowired
    private ActiveMQPublishConnectorConfiguration configuration;
    @Autowired(required = false)
    private List<ConnectorCustomizer<ActiveMQPublishComponent>> customizers;
    ...
    @PostConstruct
    public void postConstructActiveMQPublishComponent() {
        Map<String, Object> parameters = new HashMap<>();
                    ...
                    for (ConnectorCustomizer<ActiveMQPublishComponent> customizer : customizers) {
                        boolean useCustomizer = (customizer instanceof HasId)
                                ? HierarchicalPropertiesEvaluator.evaluate(
                                        applicationContext.getEnvironment(),
                                        "camel.connector.customizer",
                                        "camel.connector.activemq-publish."
                                                + entry.getKey()
                                                + ".customizer",
                                        ((HasId) customizer).getId())
                                : HierarchicalPropertiesEvaluator.evaluate(
