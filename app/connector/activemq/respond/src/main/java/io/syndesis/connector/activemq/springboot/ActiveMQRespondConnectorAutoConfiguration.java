...
import io.syndesis.connector.activemq.ActiveMQRespondComponent;
...
@EnableConfigurationProperties(ActiveMQRespondConnectorConfiguration.class)
public class ActiveMQRespondConnectorAutoConfiguration {

    private static final Logger LOGGER = LoggerFactory
            .getLogger(ActiveMQRespondConnectorAutoConfiguration.class);
    @Autowired
    private ApplicationContext applicationContext;
    @Autowired
    private CamelContext camelContext;
    @Autowired
    private ActiveMQRespondConnectorConfiguration configuration;
    @Autowired(required = false)
    private List<ConnectorCustomizer<ActiveMQRespondComponent>> customizers;
    ...
    @PostConstruct
    public void postConstructActiveMQRespondComponent() {
        Map<String, Object> parameters = new HashMap<>();
        for (Map.Entry<String, ActiveMQRespondConnectorConfigurationCommon> entry : configuration
                    ...
                    for (ConnectorCustomizer<ActiveMQRespondComponent> customizer : customizers) {
                        boolean useCustomizer = (customizer instanceof HasId)
                                ? HierarchicalPropertiesEvaluator.evaluate(
                                        applicationContext.getEnvironment(),
                                        "camel.connector.customizer",
                                        "camel.connector.activemq-respond."
                                                + entry.getKey()
                                                + ".customizer",
                                        ((HasId) customizer).getId())
                                : HierarchicalPropertiesEvaluator.evaluate(
