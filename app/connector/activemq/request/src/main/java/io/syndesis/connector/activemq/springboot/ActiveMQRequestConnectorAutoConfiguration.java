...
import io.syndesis.connector.activemq.ActiveMQRequestComponent;
...
@EnableConfigurationProperties(ActiveMQRequestConnectorConfiguration.class)
public class ActiveMQRequestConnectorAutoConfiguration {

    private static final Logger LOGGER = LoggerFactory
            .getLogger(ActiveMQRequestConnectorAutoConfiguration.class);
    @Autowired
    private ApplicationContext applicationContext;
    @Autowired
    private CamelContext camelContext;
    @Autowired
    private ActiveMQRequestConnectorConfiguration configuration;
    @Autowired(required = false)
    private List<ConnectorCustomizer<ActiveMQRequestComponent>> customizers;
    ...
    @PostConstruct
    public void postConstructActiveMQRequestComponent() {
        Map<String, Object> parameters = new HashMap<>();
                    ...
                    for (ConnectorCustomizer<ActiveMQRequestComponent> customizer : customizers) {
                        boolean useCustomizer = (customizer instanceof HasId)
                                ? HierarchicalPropertiesEvaluator.evaluate(
                                        applicationContext.getEnvironment(),
                                        "camel.connector.customizer",
                                        "camel.connector.activemq-request."
                                                + entry.getKey()
                                                + ".customizer",
                                        ((HasId) customizer).getId())
                                : HierarchicalPropertiesEvaluator.evaluate(
