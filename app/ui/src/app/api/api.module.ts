import { ModuleWithProviders, NgModule, Injector } from '@angular/core';
import { CommonModule } from '@angular/common';
import { HttpClientModule } from '@angular/common/http';

import { ApiHttpService, ApiConfigService, API_ENDPOINTS, Endpoints } from '@syndesis/ui/platform';
import * as SYNDESIS_PROVIDERS from './providers';

@NgModule({
  imports: [CommonModule, HttpClientModule],
})
export class ApiModule {
  private static staticInjector: Injector;

  constructor(private injector: Injector) {
    ApiModule.staticInjector = injector;
  }

  static forRoot(apiEndpoints?: Endpoints): Array<ModuleWithProviders> {
    return [{
      ngModule: ApiModule,
      providers: [
        {
          provide: API_ENDPOINTS,
          multi: true,
          useValue: apiEndpoints || {}
        },
        SYNDESIS_PROVIDERS.ApiConfigProviderService,
        {
          provide: ApiConfigService,
          useClass: SYNDESIS_PROVIDERS.ApiConfigProviderService
        },
        SYNDESIS_PROVIDERS.ApiHttpProviderService,
        {
          provide: ApiHttpService,
          useClass: SYNDESIS_PROVIDERS.ApiHttpProviderService
        },
      ]},
    ];
  }

  static forChild(apiEndpoints: Endpoints): Array<ModuleWithProviders> {
    const apiConfigService: ApiConfigService = ApiModule.staticInjector.get(ApiConfigService);
    apiConfigService.registerEndpoints(apiEndpoints);

    return [{
      ngModule: ApiModule
    }];
  }
}
