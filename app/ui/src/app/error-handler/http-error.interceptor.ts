...
export class HttpErrorInterceptor implements HttpInterceptor {
  ...
  constructor(private notificationService: NotificationService, private i18NService: I18NService) {
    combineLatest(
      ...
      this.errorMessages = { httpConnectFailureHeader, httpConnectFailureMessage };
    });
  }
    ...
    return next.handle(request).do((event: HttpEvent<any>) => event, (error: any) => {
      if (error instanceof HttpErrorResponse) {
        ...
        switch (error.status) {
          ...
          case 503:
          case 502:
            if (this.notificationService.getNotifications().find(n => n.header === this.errorMessages.httpConnectFailureHeader)) {
