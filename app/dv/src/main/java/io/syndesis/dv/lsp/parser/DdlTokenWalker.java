...
public class DdlTokenWalker implements SQLParserConstants {
    ...
    public Token findToken(Position position, STATEMENT_TYPE statementType) {
        ...
        if( tokens.length > 0 ) {
            ...
            // Case where the cursor is located at least 1 character after the first token (open space)
    ...
    class TokenCursorLocator {
        ...
        /*
         * Return if cursor is after last token and not in next token
         * Example "CREATE VIEW abcdefg"
         *           NO   ^
         *           YES   ^
         *           
         * Example "CREATE  VIEW abcdefg"
