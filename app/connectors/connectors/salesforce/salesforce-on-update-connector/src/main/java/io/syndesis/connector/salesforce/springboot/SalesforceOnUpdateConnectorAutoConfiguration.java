...
import io.syndesis.connector.salesforce.SalesforceOnUpdateComponent;
...
@EnableConfigurationProperties(SalesforceOnUpdateConnectorConfiguration.class)
public class SalesforceOnUpdateConnectorAutoConfiguration {

    private static final Logger LOGGER = LoggerFactory
            .getLogger(SalesforceOnUpdateConnectorAutoConfiguration.class);
    @Autowired
    private ApplicationContext applicationContext;
    @Autowired
    private CamelContext camelContext;
    @Autowired
    private SalesforceOnUpdateConnectorConfiguration configuration;
    @Autowired(required = false)
    private List<ConnectorCustomizer<SalesforceOnUpdateComponent>> customizers;
    ...
    @PostConstruct
    public void postConstructSalesforceOnUpdateComponent() {
        Map<String, Object> parameters = new HashMap<>();
        for (Map.Entry<String, SalesforceOnUpdateConnectorConfigurationCommon> entry : configuration
                    ...
                    for (ConnectorCustomizer<SalesforceOnUpdateComponent> customizer : customizers) {
                        boolean useCustomizer = (customizer instanceof HasId)
                                ? HierarchicalPropertiesEvaluator.evaluate(
                                        applicationContext.getEnvironment(),
                                        "camel.connector.customizer",
                                        "camel.connector.salesforce-on-update."
                                                + entry.getKey()
                                                + ".customizer",
                                        ((HasId) customizer).getId())
                                : HierarchicalPropertiesEvaluator.evaluate(
