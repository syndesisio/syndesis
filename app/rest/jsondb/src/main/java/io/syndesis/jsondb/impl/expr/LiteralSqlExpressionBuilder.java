...
class LiteralSqlExpressionBuilder extends SqlExpressionBuilder {
    ...
    public void build(StringBuilder sql, ArrayList<Consumer<Query<Map<String, Object>>>> binds, AtomicInteger bindCounter) {
        ...
        binds.add(query -> {
            if( value == null  ) {
                query.bind("f" + b1, (String)null);
            } else if( value == Boolean.FALSE ) {
                query.bind("f" + b1, "false");
            } else if( value == Boolean.TRUE ) {
                query.bind("f" + b1, "true");
            } else if( value.getClass() == String.class ) {
                query.bind("f" + b1, value);
            } else if( value instanceof Byte || value instanceof Short || value instanceof Integer || value instanceof Long ) {
                Number n = (Number) value;
                String value = toLexSortableString(n.longValue());
                query.bind("f" + b1, value);
            } else {
                // TODO: handle the float case.
                query.bind("f" + b1, value.toString());
            }
        });
    }
}
