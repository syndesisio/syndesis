...
public class GitHubServicelTCase {

    // Create a token using:
    //
    // curl https://api.github.com/authorizations --user "<USER_NAME>" --data '{"scopes":["repo", "delete_repo"],"note":"Syndesis-GitHubServicelTCase", "client_id":"<CLIENT_ID>", "client_secret":"<CLIENT_SECRET>" }'
    ...
    public void testGetApiUser() throws IOException {
        ...
        System.out.println("GitHub user for testing is " + apiUser);
        Assert.assertEquals("syndesis-test", apiUser);
    }
    ...
    public void testCreateNewRepository() throws IOException {
        ...
        Assert.assertEquals(testRepo, repository.getName());
 ...
 // Requires repo scope
    ...
    public void testProjectCommit()
        ...
        try {
            ...
            Assert.assertEquals(commit.getId(), lastCommit.getId());
