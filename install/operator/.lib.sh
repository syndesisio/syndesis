    ...
    "go")
        ...
        for GOARCH in amd64 ; do
          for GOOS in linux darwin windows ; do
            export GOARCH GOOS
            echo building ./dist/${GOOS}-${GOARCH}/syndesis executable
            go build  "$@" -o ./dist/${GOOS}-${GOARCH}/syndesis \
                -gcflags all=-trimpath=${GOPATH} -asmflags all=-trimpath=${GOPATH} -mod=vendor \
                ./cmd/manager
          done
        done
        mkdir -p ./build/_output/bin
        cp ./dist/linux-amd64/syndesis ./build/_output/bin/syndesis
...
RUN GOOS=linux   GOARCH=amd64 go build $OPTS -o /dist/linux-amd64/syndesis    -gcflags all=-trimpath=\${GOPATH} -asmflags all=-trimpath=\${GOPATH} -mod=vendor github.com/syndesisio/syndesis/install/operator/cmd/manager
RUN GOOS=darwin  GOARCH=amd64 go build $OPTS -o /dist/darwin-amd64/syndesis   -gcflags all=-trimpath=\${GOPATH} -asmflags all=-trimpath=\${GOPATH} -mod=vendor github.com/syndesisio/syndesis/install/operator/cmd/manager
RUN GOOS=windows GOARCH=amd64 go build $OPTS -o /dist/windows-amd64/syndesis  -gcflags all=-trimpath=\${GOPATH} -asmflags all=-trimpath=\${GOPATH} -mod=vendor github.com/syndesisio/syndesis/install/operator/cmd/manager
EODockerfile
        ...
        for GOARCH in amd64 ; do
          for GOOS in linux darwin windows ; do
            echo extracting executable to ./dist/${GOOS}-${GOARCH}/syndesis
            mkdir -p ./dist/${GOOS}-${GOARCH}
            docker run "${BUILDER_IMAGE_NAME}" cat /dist/${GOOS}-${GOARCH}/syndesis > ./dist/${GOOS}-${GOARCH}/syndesis
          done
        done
        chmod a+x ./dist/*/syndesis
        mkdir -p ./build/_output/bin
        cp ./dist/linux-amd64/syndesis ./build/_output/bin/syndesis
    ;;
    *)
