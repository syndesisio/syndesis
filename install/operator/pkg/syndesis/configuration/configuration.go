...
type Config struct {
	...
	Tag                        string
	ProductVersion             string         // the commercial product version: 7.5, 7.6, this is calculated value based on the --image.tag build parameter
...
func GetProperties(file string, ctx context.Context, client client.Client, syndesis *v1beta1.Syndesis) (*Config, error) {
	...
	if configuration.Productized {
		configuration.Tag = pkg.DefaultOperatorTag
		configuration.ProductVersion = ProductVersionOfTag(pkg.DefaultOperatorTag)
		log.Info("Configure productized versions", "configuration.Tag", configuration.Tag, "configuration.ProductVersion", configuration.ProductVersion)
	}
...
/*
/ Return the Commercial product version of Red Hat Fuse Online.

For operator tag version x.y the product version is x+6.y
Example:
input 1.6, returns: 7.6
input 1.7-12, returns: 7.7

If the input parameter is only a string "latest", it returns the same value "latest".
It discards the patch level version number.
- operatorTagVersion: A string value: example 1.6, 1.7-12
*/
func ProductVersionOfTag(ver string) string {
	if isNumber(ver) {
		i := strings.Index(ver, "-")
		if i > -1 {
			// strips the number after the "-"
			ver = ver[0:i]
		}
		i = strings.Index(ver, ".")
		// retrieve the major number
		major, _ := strconv.Atoi(ver[0:i])
		// retrieve the minor number
		minor := ver[i+1 : len(ver)]
		major = 6 + major
		ver = strconv.Itoa(major) + "." + minor
	}
	return ver
}

// ignores "-" and "." and validates if the input parameter is number
// the reasoning is to check if a version of 1.7-12, stripped of -. returns only an integer
func isNumber(ver string) bool {
