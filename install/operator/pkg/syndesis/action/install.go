...
func (a *install) Execute(scheme *runtime.Scheme, cl client.Client, syndesis *v1alpha1.Syndesis) error {
	...
	syndesisRoute, err := installSyndesisRoute(cl, syndesis, list, autoGenerateRoute)
	if err != nil {
		return err
	}
	...
	if autoGenerateRoute {
		...
		err = removeAutoGeneratedAnnotation(cl, syndesisRoute)
		if err != nil {
			return err
		}
...
func installServiceAccount(cl client.Client, syndesis *v1alpha1.Syndesis) (string, error) {
	...
	err := cl.Create(context.TODO(), sa)
	if err != nil && !k8serrors.IsAlreadyExists(err) {
		return "", err
	}

	return serviceaccount.GetServiceAccountToken(cl, sa.Name, syndesis.Namespace)
}
...
func installSyndesisRoute(cl client.Client, syndesis *v1alpha1.Syndesis, objects []runtime.Object, autoGenerate bool) (*v1.Route, error) {
	...
	err = cl.Create(context.TODO(), route)
	if err != nil && !k8serrors.IsAlreadyExists(err) {
		return nil, err
	}
	...
	err = cl.Get(context.TODO(), key, route)
	if err != nil {
		return nil, err
	}
...
func removeAutoGeneratedAnnotation(cl client.Client, route *v1.Route) error {
	return updateOnLatestRevision(cl, route, func(obj runtime.Object) {
